  ...
  dashboard_body = templatefile(format("%s/templates/dashboard.json", path.module),
    {
      ...
      gateway_name          = format("%s-%s", module.labels.id, "gw")
      ...
      lambda_token_fn_name  = format("%s-%s", module.labels.id, "token")
      lambda_cso_fn_name    = format("%s-%s", module.labels.id, "cso")
      lambda_stats_fn_name  = format("%s-%s", module.labels.id, "stats")
      api_lb_arn_suffix     = aws_lb.api.arn_suffix
      push_lb_arn_suffix    = aws_lb.push.arn_suffix
      api_log_group         = format("%s-%s", module.labels.id, "api")
  })
}
